...
#   -g          visual workflow: run timeline + SQL waterfall showing which
#               phase of the run is slow and which SQLs make up that phase
...
WHATIS_ONLY=0; DUMP_AE=0; SHOW_PLANS=0; SHOW_RT=0; RUN_STA=0; SHOW_FLOW=0

usage() { sed -n '2,46p' "$0" | sed 's/^# \{0,1\}//'; exit 1; }

while getopts ":n:d:o:i:m:v:N:wagprth" opt; do
  case "$opt" in
...
# =============================================================================
# STEP 3g - VISUAL WORKFLOW  (-g)  (process/runctl modes only)
# Reconstructs the run as a timeline from ASH. ACTION is null on these AE
# domains, so phases are inferred from WHEN each SQL executed, not step names.
# =============================================================================
if [ "$SHOW_FLOW" -eq 1 ]; then
if [ "$MODE" != "process" ] && [ "$MODE" != "runctl" ]; then
  banner "WORKFLOW (-g) only applies to process/runctl mode - skipped"
else
cat > "$TMPD/flow.sql" <<SQLEOF
SET FEEDBACK OFF VERIFY OFF HEADING ON TAB OFF LINESIZE 250 PAGESIZE 500 TIMING OFF
DEFINE N='&1'
DEFINE DAYS=&2
DEFINE PSOWNER='&3'
DEFINE ONEINST=&4
...
PROMPT
PROMPT =========================================================================
PROMPT === G0. TARGET RUN FOR THE WORKFLOW
PROMPT =========================================================================
PROMPT (Default = the SLOWEST run in the window. Use -i PRCS_INST for another.)
COLUMN prcsinstance FORMAT 99999999 HEADING 'PRCS_INST'
COLUMN prcsname FORMAT A13 HEADING 'PRCS_NAME'
COLUMN beg FORMAT A19 HEADING 'BEGIN'
COLUMN run_sec FORMAT 999,990 HEADING 'RUN_SEC'
...
COLUMN band FORMAT A5 HEADING 'BAND'
WITH $CORR,
tgt AS (SELECT b.* FROM banded b
         WHERE (&ONEINST > 0 AND b.prcsinstance = &ONEINST)
            OR (&ONEINST = 0 AND b.run_sec =
                 (SELECT MAX(run_sec) FROM banded))
         FETCH FIRST 1 ROWS ONLY)
SELECT t.prcsinstance, t.prcsname,
       TO_CHAR(t.beg_ts,'YYYY-MM-DD HH24:MI:SS') beg,
       ROUND(t.run_sec) run_sec, ROUND(t.run_sec/60,1) run_min, t.band
  FROM tgt t;

PROMPT
PROMPT =========================================================================
PROMPT === G1. RUN TIMELINE - where does the time go? (40 buckets)
PROMPT =========================================================================
PROMPT Each row = 1/40th of the run. BAR = concurrent DB activity in that slice
PROMPT (longer bar = more active sessions = heavier). DOMINANT_SQL = the
PROMPT statement consuming most of that slice. Long stretches with the same
PROMPT dominant SQL and a long bar = that is the slow phase of the job.
COLUMN bkt FORMAT 990 HEADING 'SEG'
COLUMN from_min FORMAT 9,990.9 HEADING 'FROM_MIN'
COLUMN to_min FORMAT 9,990.9 HEADING 'TO_MIN'
COLUMN aas FORMAT 90.9 HEADING 'AAS'
COLUMN bar FORMAT A42 HEADING 'ACTIVITY (each # ~ activity)'
COLUMN dominant_sql FORMAT A14 HEADING 'DOMINANT_SQL'
COLUMN dom_pct FORMAT 990 HEADING 'DOM%'
COLUMN dom_event FORMAT A26 HEADING 'DOMINANT_EVENT'
WITH $CORR,
tgt AS (SELECT b.* FROM banded b
         WHERE (&ONEINST > 0 AND b.prcsinstance = &ONEINST)
            OR (&ONEINST = 0 AND b.run_sec =
                 (SELECT MAX(run_sec) FROM banded))
         FETCH FIRST 1 ROWS ONLY),
s AS (SELECT a.sql_id, NVL(a.event,'CPU') event,
             LEAST(39, FLOOR(((CAST(a.sample_time AS DATE)-CAST(t.beg_ts AS DATE))*86400)
                              / GREATEST(t.run_sec/40,1))) bkt,
             t.run_sec
        FROM ash a JOIN tgt t ON t.prcsinstance = a.prcsinstance),
per AS (SELECT bkt, COUNT(*) n, MAX(run_sec) rs FROM s GROUP BY bkt),
mx  AS (SELECT MAX(n) m FROM per),
dom AS (SELECT bkt, sql_id, pct FROM (
          SELECT bkt, sql_id,
                 ROUND(100*COUNT(*)/SUM(COUNT(*)) OVER (PARTITION BY bkt)) pct,
                 ROW_NUMBER() OVER (PARTITION BY bkt ORDER BY COUNT(*) DESC) rn
            FROM s GROUP BY bkt, sql_id) WHERE rn=1),
dev AS (SELECT bkt, event FROM (
          SELECT bkt, event, ROW_NUMBER() OVER (PARTITION BY bkt ORDER BY COUNT(*) DESC) rn
            FROM s GROUP BY bkt, event) WHERE rn=1)
SELECT p.bkt,
       ROUND(p.bkt*p.rs/40/60,1) from_min,
       ROUND((p.bkt+1)*p.rs/40/60,1) to_min,
       ROUND(p.n*10/GREATEST(p.rs/40,1),1) aas,
       RPAD('#', GREATEST(ROUND(40*p.n/NULLIF(m.m,0)),1), '#') bar,
       d.sql_id dominant_sql, d.pct dom_pct, e.event dom_event
  FROM per p CROSS JOIN mx m
  LEFT JOIN dom d ON d.bkt = p.bkt
  LEFT JOIN dev e ON e.bkt = p.bkt
 ORDER BY p.bkt;

PROMPT
PROMPT =========================================================================
PROMPT === G2. SQL WATERFALL - the job's phases in execution order
PROMPT =========================================================================
PROMPT Each row = one SQL, ordered by when it FIRST ran = the workflow order.
PROMPT The ===== bar shows WHERE in the run it was active (0% left, 100% right).
PROMPT Longest DB_TIME + widest bar = the slow section. <== HOT marks top 3.
COLUMN seq FORMAT 990 HEADING 'PH'
COLUMN sql_id FORMAT A14 HEADING 'SQL_ID'
COLUMN phv FORMAT 9999999999 HEADING 'PLAN_HASH'
COLUMN st_min FORMAT 9,990.9 HEADING 'START_MIN'
COLUMN en_min FORMAT 9,990.9 HEADING 'END_MIN'
COLUMN dbt FORMAT 999,990 HEADING 'DB_TIME_S'
COLUMN track FORMAT A62 HEADING 'POSITION IN RUN  |0%..............................100%|'
COLUMN hot FORMAT A8 HEADING 'FLAG'
WITH $CORR,
tgt AS (SELECT b.* FROM banded b
         WHERE (&ONEINST > 0 AND b.prcsinstance = &ONEINST)
            OR (&ONEINST = 0 AND b.run_sec =
                 (SELECT MAX(run_sec) FROM banded))
         FETCH FIRST 1 ROWS ONLY),
s AS (SELECT a.sql_id, a.phv,
             (CAST(a.sample_time AS DATE)-CAST(t.beg_ts AS DATE))*86400 off,
             t.run_sec rs
        FROM ash a JOIN tgt t ON t.prcsinstance = a.prcsinstance),
g AS (SELECT sql_id, MIN(phv) phv, MIN(off) st, MAX(off) en,
             COUNT(*) n, MAX(rs) rs
        FROM s GROUP BY sql_id),
r AS (SELECT g.*, ROW_NUMBER() OVER (ORDER BY n DESC) hotrk,
             ROW_NUMBER() OVER (ORDER BY st) seq
        FROM g),
top AS (SELECT * FROM r WHERE hotrk <= 20)
SELECT t.seq, t.sql_id, t.phv,
       ROUND(t.st/60,1) st_min, ROUND(t.en/60,1) en_min,
       t.n*10 dbt,
       NVL(RPAD(' ', ROUND(60*t.st/GREATEST(t.rs,1)), ' '),'')
         || RPAD('=', GREATEST(ROUND(60*(t.en-t.st)/GREATEST(t.rs,1)),1), '=') track,
       CASE WHEN t.hotrk <= 3 THEN '<== HOT' END hot
  FROM top t
 ORDER BY t.seq;
PROMPT
PROMPT Reading it: phases (PH) run top to bottom in time order. A bar hugging
PROMPT the left = early phase; hugging the right = end of run. A HOT row whose
PROMPT bar spans most of the width = one statement dominating the whole run.
PROMPT Cross-reference the HOT SQL_IDs with sections D3 (which plan line) and
PROMPT D4 (which table) to name the exact operation that makes it slow.
EXIT;
SQLEOF
banner "VISUAL WORKFLOW"
sqlplus -s "/ as sysdba" @"$TMPD/flow.sql" "$NAME_UC" "$DAYS" "$PS_OWNER_UC" "$ONE_INST"
fi
fi
